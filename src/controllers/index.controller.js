...
/*
const { cuentaBancaria2 } = require('../models/cuentaBancaria2');
*/
...
const getlogin= async (req,res) => { 
    ...
    if (!errors.isEmpty()) {
      const validaciones = errors.array()
      console.log(errors);
      res.render('login', {validaciones: validaciones})
    }else{
    ...
    .catch(error =>{
        ...
        res.render('login', {ahora:'No existen usuarios'})
    })
...
const setRetiro= async (req,res) => { 
    ...
    var currency = currency.replace(/[.]+/g,"");
    var currency = currency.replace(/[,]+/g,".");
    console.log(currency)
    var saldobancario = Number(currency.replace(/[^0-9\.-]+/g,""));
    ...
    .then(result => {
      ...
      /*
      cuentaBancaria2.update({ saldo_cuentabancaria: (saldobancario-saldo2) }, {
        where: {
            id_cuentabancaria: cuenta,
            saldo_cuentabancaria: {
            [Op.gte]: saldo2,
